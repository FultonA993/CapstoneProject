...
class MyApplication6(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.app = app
        
        # Set the window title and size
        self.setWindowTitle('Forsyth Family Medical Practice Licenses')
        ...
        self.label4 = QtWidgets.QLabel('License Details:')
        self.label4.setStyleSheet('color: white;')
        self.button1 = QtWidgets.QPushButton("Send Renew Notification", self)
        self.button1.setStyleSheet("background-color: #ADE792;")
        self.button1.clicked.connect(show_notification)
        #self.button1.clicked.connect(self.displayMessage)
    ...
    def search_item(self, search_string):
        ...
        if record is not None:
            ...
            if len(text) > 6:
                self.list_box.setStyleSheet("color: red; background-color: #F3ECB0")
            else:
                self.list_box.setStyleSheet("background-color: #F3ECB0")  # reset the style sheet
...
def show_notification():
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Renew License")
        msg_box.setText("Notification sent to employer and employee. ")
        msg_box.exec_()
